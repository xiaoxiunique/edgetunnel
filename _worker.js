					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						...
						if (订阅类型 === 'mixed') {
							...
							if (!url.searchParams.has('sub') && config_JSON.优选订阅生成.local) { // 本地生成订阅
								const 地区筛选配置 = config_JSON.优选订阅生成.本地IP库.地区筛选;
								const 自动优选数量 = 地区筛选配置?.启用 ? 地区筛选配置.自动优选数量 : config_JSON.优选订阅生成.本地IP库.随机数量;
								const ADD文本 = await env.KV.get('ADD.txt');
								const 自定义优选列表 = ADD文本 ? await 整理成数组(ADD文本) : [];
								const 自动优选列表 = config_JSON.优选订阅生成.本地IP库.随机IP ? (
									await 生成随机IP(request, 自动优选数量, config_JSON.优选订阅生成.本地IP库.指定端口)
								)[0] : [];
								const 完整优选列表 = config_JSON.优选订阅生成.本地IP库.随机IP
									? 自动优选列表.concat(自定义优选列表)
									: (自定义优选列表.length > 0 ? 自定义优选列表 : (
							...
							完整优选IP = 按地区筛选优选IP(完整优选IP, config_JSON.优选订阅生成.本地IP库.地区筛选);
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	if (!config_JSON.优选订阅生成.本地IP库.地区筛选) config_JSON.优选订阅生成.本地IP库.地区筛选 = 默认配置JSON.优选订阅生成.本地IP库.地区筛选;
	if (!Array.isArray(config_JSON.优选订阅生成.本地IP库.地区筛选.地区顺序)) config_JSON.优选订阅生成.本地IP库.地区筛选.地区顺序 = 默认配置JSON.优选订阅生成.本地IP库.地区筛选.地区顺序;
	config_JSON.优选订阅生成.本地IP库.地区筛选.自动优选数量 = Math.max(0, parseInt(config_JSON.优选订阅生成.本地IP库.地区筛选.自动优选数量, 10) || 默认配置JSON.优选订阅生成.本地IP库.地区筛选.自动优选数量);
	config_JSON.优选订阅生成.本地IP库.地区筛选.每地区最多节点 = Math.max(1, parseInt(config_JSON.优选订阅生成.本地IP库.地区筛选.每地区最多节点, 10) || 默认配置JSON.优选订阅生成.本地IP库.地区筛选.每地区最多节点);
...
function 识别节点地区(节点) {
	let 备注 = String(节点 || '').split('#').slice(1).join('#') || '';
	try { 备注 = decodeURIComponent(备注) } catch { }
	const 地区规则 = [
		{ name: '香港', pattern: /香港|Hong\s*Kong|\bHK\b|HKG/i },
		{ name: '新加坡', pattern: /新加坡|Singapore|\bSG\b|SIN/i },
		{ name: '日本', pattern: /日本|Japan|Tokyo|Osaka|Fukuoka|\bJP\b|NRT|HND|KIX|FUK/i },
		{ name: '美国', pattern: /美国|United\s*States|USA|\bUS\b|Los\s*Angeles|San\s*Jose|Seattle|Dallas|Chicago|Ashburn|Newark|LAX|SJC|SEA|DFW|ORD|IAD|EWR/i },
		{ name: '台湾', pattern: /台湾|Taiwan|Taipei|\bTW\b|TPE/i },
		{ name: '韩国', pattern: /韩国|Korea|Seoul|\bKR\b|ICN/i },
	];
	return 地区规则.find(({ pattern }) => pattern.test(备注))?.name || '其他';
}

function 是自动随机CF优选节点(节点) {
	let 备注 = String(节点 || '').split('#').slice(1).join('#') || '';
	try { 备注 = decodeURIComponent(备注) } catch { }
	return /(?:^|\s)CF(?:官方|移动|联通|电信)优选\d+$/i.test(备注.trim());
}

function 按地区筛选优选IP(优选IP列表, 地区筛选配置 = {}) {
	if (!地区筛选配置?.启用) return 优选IP列表;
	const 每地区最多节点 = Math.max(1, parseInt(地区筛选配置.每地区最多节点, 10) || 4);
	const 地区顺序 = Array.isArray(地区筛选配置.地区顺序) && 地区筛选配置.地区顺序.length > 0
		? 地区筛选配置.地区顺序
		: ['香港', '新加坡', '日本', '美国'];
	const 地区排序 = new Map(地区顺序.map((地区, index) => [地区, index]));
	const 地区计数 = new Map();
	return 优选IP列表
		.map((节点, index) => ({ 节点, index, 地区: 识别节点地区(节点), 自动随机CF优选节点: 是自动随机CF优选节点(节点) }))
		.filter(({ 地区, 自动随机CF优选节点 }) => {
			if (地区 === '其他' || 自动随机CF优选节点) return true;
			if (!地区排序.has(地区)) return false;
			const 当前数量 = 地区计数.get(地区) || 0;
			if (当前数量 >= 每地区最多节点) return false;
			地区计数.set(地区, 当前数量 + 1);
			return true;
		})
		.sort((a, b) => (地区排序.get(a.地区) ?? 999) - (地区排序.get(b.地区) ?? 999) || a.index - b.index)
		.map(({ 节点 }) => 节点);
}
